...
namespace MauiApp2.Services
{
    ...
    {
        public BLEDevice NewDeviceCandidateFromHomePage { get; set; } = new();
        public List<BLEDevice> DeviceList { get; private set; }
        ...
        public async Task<List<BLEDevice>> ScanForDevicesAsync()
        {
            ...
            try
            {
                ...
                {
                    BLEDevice device = DeviceList.FirstOrDefault(d => d.Id == systemDevice.Id);
                    if (device == null)
                    {
        ...
        {
            BLEDevice device = DeviceList.FirstOrDefault(d => d.Id == e.Device.Id);
            if (device == null)
            {
                DeviceList.Add(new BLEDevice
                {
